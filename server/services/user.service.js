...
export const verifyInviteToken = async (token) => {
    ...
    if(!tokenData) return { success: false, message: 'Invalid token', statusCode: 400 };

    // check if token is expired
    if(tokenData.expires_at < new Date()) return { success: false, message: 'Token expired', statusCode: 400 };

    // check if token is consumed
    if(tokenData.is_consumed) return { success: false, message: 'Token already consumed', statusCode: 400 };
    ...
    if(tokenData.created_by !== userId) return { success: false, message: 'Invalid token', statusCode: 400 };
    ...
    return { success: true, message: 'Token verified', data: tokenData, statusCode: 200 };
}
